...
import base64, csv, hashlib, hmac, io, json, os, re, secrets, sqlite3, sys, time, threading
from database import PostgresConnection, ConfigurationError, LocalConnection
...
CLOUD = bool(os.environ.get('VERCEL'))
DATABASE_URL = os.environ.get('DATABASE_URL', '').strip()
MAX_UPLOAD = 3_000_000
if not CLOUD and not DATABASE_URL:
...
def ensure_ready():
    ...
    with _ready_lock:
        ...
        if CLOUD and not DATABASE_URL:
            raise ConfigurationError('Falta conectar o banco online. Configure DATABASE_URL nas variáveis da Vercel e faça um novo deploy.')
        if CLOUD and len(os.environ.get('PRATES_SETUP_KEY','')) < 24:
            raise ConfigurationError('Configure PRATES_SETUP_KEY na Vercel com uma chave privada de pelo menos 24 caracteres e faça um novo deploy.')
        initialize()
        _ready = True
...
def conn():
    if DATABASE_URL: return PostgresConnection(DATABASE_URL, SQLITE_SCHEMA)
    if CLOUD: raise ConfigurationError('DATABASE_URL não configurada. O banco local não pode ser usado na Vercel.')
    c = sqlite3.connect(DB, timeout=20, factory=LocalConnection)
...
class App(BaseHTTPRequestHandler):
    ...
    def send(self,status,obj,ctype='application/json; charset=utf-8',headers=None):
        ...
        if CLOUD and len(raw)>4_200_000:
            return self.send(413, {'error':'A exportação excede o limite de resposta da Vercel. Use o backup do provedor PostgreSQL, que inclui os anexos, ou exporte menos registros.'})
    ...
    def body(self):
        ...
        if size>4_100_000: raise ValueError('Arquivo ou conteúdo muito grande. Limite: 3 MB por anexo.')
    ...
    def handle_request(self,method):
        try:
            ...
            ensure_ready()
            with conn() as c:
                if DATABASE_URL and method!='GET':c.execute('BEGIN IMMEDIATE')
                if path=='/api/session':
                    u=self.session(c)
                    return self.send(200,{'setup':not bool(c.execute('SELECT 1 FROM users').fetchone()),'user':user_public(u) if u else None,'csrf':u['csrf'] if u else None,'setupKeyRequired':bool(os.environ.get('PRATES_SETUP_KEY')),'uploadLimit':MAX_UPLOAD})
                if path in ['/api/setup','/api/login','/api/recover'] and method=='POST':
                    ...
                    if path=='/api/setup':
                        ...
                        setup_key=os.environ.get('PRATES_SETUP_KEY','')
                        if setup_key and not hmac.compare_digest(str(b.get('setup_key','')),setup_key):
                            return self.send(403,{'error':'Chave de instalação inválida. Use a PRATES_SETUP_KEY que você configurou na Vercel.'})
                ...
                if path=='/api/backup' and method=='GET':
                    ...
                    if DATABASE_URL:
                        # Serialize snapshot reads with application writes.
                        c.execute('BEGIN IMMEDIATE')
                        c.backup(mem)
                    else:
                        c.backup(mem)
                        mem.execute('DELETE FROM sessions');mem.commit()
                    with zipfile.ZipFile(dest,'w',zipfile.ZIP_DEFLATED) as z:
                        z.writestr('prates.sqlite3',mem.serialize())
                        if DATABASE_URL:
                            for blob in c.execute('SELECT id,content FROM upload_blobs'):
                                z.writestr('uploads/'+blob['id'],bytes(blob['content']))
                        else:
                ...
                if path.startswith('/api/attachments'):
                    if method=='POST' and path=='/api/attachments':
                        ...
                        if len(raw)>MAX_UPLOAD:raise ValueError('O limite por arquivo é 3 MB.')
                        aid=secrets.token_hex(16);name=Path(str(b.get('name','arquivo')).replace('\\','/')).name[:160]
                        c.execute('INSERT INTO attachments VALUES(?,?,?,?,?)',(aid,r['id'],name,len(raw),now()))
                        if DATABASE_URL:c.execute('INSERT INTO upload_blobs VALUES(?,?)',(aid,raw))
                        else:(DATA/'uploads'/aid).write_bytes(raw)
                        audit(c,u['id'],'Anexo adicionado',r['entity'],r['id'],{'name':name});c.commit();return self.send(201,{'id':aid})
                    ...
                    if method=='GET':
                        ...
                        content=bytes(c.execute('SELECT content FROM upload_blobs WHERE id=?',(aid,)).fetchone()[0]) if DATABASE_URL else (DATA/'uploads'/aid).read_bytes()
                        return self.send(200,content,'application/octet-stream',{'Content-Disposition':"attachment; filename*=UTF-8''"+quote(a['name'])})
        ...
        except ConfigurationError as e:self.send(503,{'error':str(e),'code':'CONFIGURATION_REQUIRED'})
            ...
            print('Erro interno:',type(e).__name__,file=sys.stderr)
...
if __name__=='__main__':
    ensure_ready();host=os.environ.get('PRATES_HOST','127.0.0.1');port=int(os.environ.get('PRATES_PORT','8765'))
